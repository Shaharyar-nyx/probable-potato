    ...
    const smoothScroll = () => {
      ...
      Array.from(cards).forEach((card) => {
        ...
        const blurRadius = Math.min(
          maxBlur,
          (distanceFromCenter / viewportCenter) * maxBlur
        );
        ...
        if (isVisible) {
          const progress =
            ...
            Math.min(
              1,
              Math.abs(distanceFromCenter) / (window.innerHeight * 0.5)
            );
          (card as HTMLElement).style.opacity = progress.toString();
          (card as HTMLElement).style.transform =
            `scale(${0.8 + 0.2 * progress}) translateY(${50 * (1 - progress)}px)`;
  ...
  return (
    <div ref={containerRef} className={styles.container}>
      ...
      {brandMissionContent.teamMembers.map((member, index) => (
        <div
          key={member.name}
          className={`${styles.memberCard} ${styles["member" + (index + 1)]}`}
        >
          <img
          ...
          <div className={styles.memberInfo}>
            <div className={`${styles.memberName} font-primarySemiBold`}>
              {member.name}
            </div>
